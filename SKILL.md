...
本技能零**硬**外部依赖：验证流程默认由推理本身执行（手算/符号代入/数值抽样），不依赖任何脚本、环境或插件。若环境中存在 Python + SymPy/SciPy，复杂题可自动升级为一次符号复核；环境不可用时不影响主流程，降级为手算并显式声明（见 `modules/computation.md`）。

**工具门禁（Level 1–4）**：
- L1 简单题：纯手算，禁止调用外部工具。
- L2 中等题：手算为主；环境可用时允许执行一次符号复核，环境不可用则手算。
- L3 复杂题：自动升级验证——环境可用时自动执行一次符号复核（ODE、特征值、积分、归一化等），不可用时手算并声明“未使用工具”。
- L4 用户要求高置信度/复核：在 L3 基础上执行 `review_engine.md` 的 P1–P5 复核；白纸重推按该模块规则。

工具调用必须以真实运行结果为据；未实际调用工具时禁止声称“已用 SymPy/Python 验证”。用户明确要求只用手算时，任何级别都禁用工具。

## 适用范围

**当前覆盖**：理论力学、电磁学、基础量子力学；其余本科方向列入覆盖路线图，尚未覆盖（见下）。
...
补齐前，“undergrad physics”指目标定位而非已覆盖全集。

## 范围外

- 本科其他方向：热学、统计物理、光学/波动、狭义相对论等（当前未覆盖，列入覆盖路线图；遇到时先声明范围外）。
...
3. **推导（Derive）**：分步推导，每步注明依据；先以符号推导、最后代入数值。在关键中间结果、近似/表示切换和最终结果前，执行最适合的快速核验并内联记录。矩阵、特征值、矩阵幂、递推等线性代数子问题：默认按 J 一致性手算；复杂题可按 L3 自动用 SymPy 复核；调用 `math-skill` 时注明借用了哪些结果。完成判据：每步有依据且可独立核验，符号推导在数值代入前完成。
4. **验证（Verify）**：从 `modules/verification_engine.md` 选择最小充分验算集：通常 F 与 B，再加一项独立检查；默认总项数 3，用户要求高置信度时最多 5；每个不适用项写出物理原因。E 数值抽样：简单/中等题只做手算可复核的小样例；复杂题按 L3 可在环境可用时执行一次数值脚本，并记录真实结果。领域模块标为必做的 J 不可替代。逐条给出实际检查与 PASS/FAIL；失败则进入回溯修正协议。完成判据：所有已选检查 PASS，所有 N/A 均有理由，FAIL 0 项。
5. **复核（Review，可选）**：用户要求高置信度或题目复杂时，按 `modules/review_engine.md` 做独立复核并附 P1–P5 摘要；白纸重推仅在用户明确要求或发现病理时执行。不要求时跳过。完成判据：触发时 P1–P5 全部 PASS，或白纸重推后 FAIL 0 项。
...
- 工具使用按 L1–L4 门禁执行；只有真实执行工具时才可写“已用 SymPy/Python 验证”。
- **输出止损**：复杂/多问问题按问分节，先给每问最终结果和一行验算；任何情况下不得返回空答案。
...
| 符号计算 | `modules/computation.md` | 可选 SymPy/SciPy 复核配方（L2–L3 使用，无依赖降级友好） |
