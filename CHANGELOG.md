...
- 默认简洁路径：按模板最小化执行范围，简单题收紧输出预算；符号复核默认关闭（仅用户要求时一次），默认验算 3 项、最多 5 项；LaTeX 模式默认不写文件、简单题 ≤60 行；未获要求禁止调用/声称 SymPy；复核默认 P1-P5 摘要、白纸重推默认不做；新增输出止损（禁止空答）与禁止自动数值积分；范围外题目先声明不硬解
