...
本模块是**可选**复核工具，不是主流程的一部分。仅当**环境中存在 Python + SymPy/SciPy 且用户要求**时使用。

**默认忽略本模块**：未经用户明确要求，不得调用 Python/SymPy，也不得在输出中声称已用 SymPy 验证。

**纪律**：配方输出只是交叉验证，不能代替主流程的 PASS/FAIL 记录；结果与手算不一致时按回溯修正协议处理。
