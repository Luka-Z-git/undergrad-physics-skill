...
本模块覆盖本科基础量子力学：波函数与概率诠释、定态薛定谔方程、一维势阱/势垒、谐振子、算符与对易关系、角动量、氢原子能级入门。每个子域给出：识别特征、建模步骤、验证组合（F/D/B/C/L/E/I/J，定义见 `verification_engine.md`）、必须检查的适用条件、常见错误表。引用任何定律时先声明其适用条件；验证未通过不得输出最终答案。
...
- 非相对论、无自旋-轨道耦合、无外场；精细结构与塞曼效应不在本模块范围。
...
| 错误 | 正确 |
|---|---|
| $\hat p$ 符号写反 | $\hat p=-i\hbar\nabla$ |
| 算符不可交换仍交换 | 先展开对易子 |
| 波函数不归一 | 束缚态归一化必做 |
| $E=h\nu$ 与 $E=\hbar\omega$ 混淆 | 同一能量，$\omega=2\pi\nu$ |
| 用经典可交换性推理量子 | 量子力学算符按对易规则 |
| 数值结果不写单位 | 数值答案一律带单位 |
