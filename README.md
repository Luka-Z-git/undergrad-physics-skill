...
- **内置验证引擎**：8 种验证方法（F 量纲 / D 定义域 / B 回代 / C 守恒量 / L 极限特例 / E 数值抽样 / I 独立方法 / J 一致性）。标准解答必查：量纲、极限/特例、回代、守恒量（适用时），每条 PASS 必须附可复核的具体检查；验证失败自动回溯修正，无法验证时如实声明。
...
硬性规则：标准解答必查 F 量纲、L 极限/特例、B 回代、C 守恒量（适用时）；任何 FAIL 回溯修正；禁止伪造验证通过；两次修正仍失败则换独立方法，仍失败则明确声明无法给出已验证答案。完整方法与回溯协议以 `modules/verification_engine.md` 为准。
